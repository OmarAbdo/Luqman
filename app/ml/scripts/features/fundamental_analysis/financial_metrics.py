...
def calculate_revenue_growth(income_statement):
    try:
        revenue_current = income_statement.loc["Total Revenue"][0]
        revenue_previous = income_statement.loc["Total Revenue"][1]
        return (
...
def calculate_earnings_growth(income_statement):
    try:
        net_income_current = income_statement.loc["Net Income"][0]
        net_income_previous = income_statement.loc["Net Income"][1]
        return (
...
def calculate_operating_cash_flow(cash_flow):
    try:
        return cash_flow.loc["Operating Cash Flow"][0]
    except KeyError:
        return None
...
def calculate_free_cash_flow(cash_flow):
    try:
        return cash_flow.loc["Free Cash Flow"][0]
    except KeyError:
        return None
